...
import type { RGB } from "./dominant-color";

export function getReadableTextColor(bg: RGB, candidates: RGB[] = []): string {
    ...
    const contrast = (c1: RGB, c2: RGB): number => {
    ...
    const adjust = (c: RGB, amt: number): RGB => ({
        r: clamp(c.r + amt),
        g: clamp(c.g + amt),
        b: clamp(c.b + amt)
    });
